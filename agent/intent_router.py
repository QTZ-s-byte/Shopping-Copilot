...
class IntentRouter:
    ...
    def classify(self, user_message: str, state: SessionState) -> IntentResult:
        ...
        clarification_attribute = self.clarification_policy.suggest(
            state, intent, hard, soft, negative
        )
    ...
    def _detect_override(
        ...
        if hit is None:
            return False, ""
        if self._goal_changed(state, slots):
            return True, hit
        return False, ""
    ...
    def _detect_no_preference_attributes(self, message: str) -> Tuple[str, ...]:
        ...
        for attr, name in ATTRIBUTE_NAMES.items():
            if re.search(r"\b(?:no|without)\s+" + re.escape(name) + r"\b", text):
                found.append(attr)
            elif re.search(
                r"\b" + re.escape(name) + r"\s+(?:doesn'?t|doesnt)\s+matter\b", text
            ):
                found.append(attr)
